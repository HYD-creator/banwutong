...
function homeworkPage() {
  ...
  state.students.forEach(s=>counts[state.homeworkStatus?.[`${current.id}:${s.id}`]||'未交']++);
  return shell(`<div class="hero"><div><span class="eyebrow">${esc(current.subject)} · ${esc(assignmentDate(current))}</span><h1>${esc(current.title)}</h1><p class="sub">按学生记录已交、未交或请假。</p></div><div class="row"><button class="btn" data-action="back-homework-list">返回作业列表</button><button class="btn" data-action="export-homework">导出 CSV</button></div></div><div class="card pad"><div class="toolbar"><strong>提交情况</strong><div class="toolbar-spacer"></div><span class="pill green">已交 ${counts.已交}</span><span class="pill orange">未交 ${counts.未交}</span><span class="pill">请假 ${counts.请假}</span></div><div class="table-wrap"><table><thead><tr><th>学生</th><th>学号</th><th>提交状态</th></tr></thead><tbody>${state.students.map(s=>{const status=state.homeworkStatus?.[`${current.id}:${s.id}`]||'未交';return `<tr><td><strong>${esc(s.name)}</strong></td><td>${esc(s.number||'—')}</td><td><div class="status-switch">${['已交','未交','请假'].map(x=>`<button class="btn small ${status===x?'primary':''}" data-homework-status="${current.id}|${s.id}|${x}">${x}</button>`).join('')}</div></td></tr>`}).join('')}</tbody></table></div></div>`,"homework");
}
...
function exportHomework(){const a=(state.assignments||[]).find(x=>x.id===state.homeworkDetailId);if(!a)return toast('请先打开一项作业');downloadCsv(`${currentClass()}_${a.subject}_${a.title}.csv`,[['学生','学号','状态'],...state.students.map(s=>[s.name,s.number||'',state.homeworkStatus?.[`${a.id}:${s.id}`]||'未交'])]);toast('作业提交情况已导出');}
bootstrap();
