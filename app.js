...
function displayHomeworkPage(){const today=localDateValue();const assignments=(state.assignments||[]).filter(item=>assignmentDate(item)===today).sort((a,b)=>String(a.createdAt).localeCompare(String(b.createdAt)));return `<div class="display-page"><div class="display-shell"><div class="display-head"><div><div class="display-title">${esc(currentClass())} · 作业提交</div><div style="opacity:.8;margin-top:4px">${today} · 仅显示未交学生</div></div><button class="btn" data-action="back-display">返回教室首页</button></div><div class="display-home">${assignments.length?`<div class="display-homework-grid">${assignments.map(item=>{const missing=state.students.filter(student=>homeworkStudentStatus(item,student.id)==='未交');return `<section class="card display-homework-card"><div class="display-homework-title"><span class="eyebrow">${esc(item.subject)}</span><h2>${esc(item.title)}</h2><span class="pill orange">未交 ${missing.length}</span></div>${missing.length?`<div class="missing-students">${missing.map(student=>`<span>${esc(student.name)}</span>`).join('')}</div>`:`<div class="all-submitted">全部已交</div>`}</section>`}).join('')}</div>`:`<div class="card empty">今天暂无作业</div>`}</div></div>${modal()}</div>`;}
