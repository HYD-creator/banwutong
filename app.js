...
function homeworkPage() {
  ...
  if(!current){
    ...
    return shell(`<div class="hero"><div><span class="eyebrow">作业管理</span><h1>每日作业</h1><p class="sub">按日期查看和添加作业，点击作业后登记学生提交情况。</p></div><div class="row"><select class="select compact-select" id="homework-stats-period">${periodOptions(statsPeriod)}</select><button class="btn" data-action="export-daily-homework">导出未交统计</button><button class="btn primary" data-action="add-assignment">＋ 新建作业</button></div></div><div class="card pad"><div class="homework-date-bar"><label for="homework-date">作业日期</label><input class="input" id="homework-date" type="date" value="${esc(selectedDate)}" /></div><div class="homework-list">${daily.length?daily.map(item=>`<button class="homework-item" data-homework-detail="${item.id}"><span><strong>${esc(item.subject)}</strong><b>${esc(item.title)}</b></span><span class="sub">查看提交情况 →</span></button>`).join(''):`<div class="empty">${selectedDate===localDateValue()?'今天':'该日期'}还没有作业，点击“新建作业”进行添加。</div>`}</div></div>`,"homework");
  }
