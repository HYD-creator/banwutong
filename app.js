...
document.addEventListener("submit", async e => {
  ...
  if(e.target.id==='assignment-form'){const item={id:crypto.randomUUID(),subject:String(f.get('subject')||'').trim(),title:String(f.get('title')||'').trim(),date:String(f.get('date')||''),createdAt:new Date().toISOString()};(state.assignments||=[]).unshift(item);state.activeAssignmentId=item.id;state.modal=null;persist();render();toast('作业已创建并同步到教室展示模式');return;}
