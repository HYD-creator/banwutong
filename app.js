...
function randomSchedule(){
  ...
  state.tasks.forEach(task=>DAYS.forEach(day=>{const available=[...state.students].filter(student=>!usedWeek.has(student.id)).sort(()=>Math.random()-.5);const chosen=available.slice(0,task.count).map(student=>student.id);chosen.forEach(id=>usedWeek.add(id));next[`${task.id}:${day}`]=chosen;}));
