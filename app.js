...
function announcementStatus(item,now=Date.now()) {
  ...
  return start>now?'scheduled':(end<=now?'history':'active');
}
...
function showNextDisplayAlert() {
  state.modal=String(state.view).startsWith('display')?pendingDisplayAlert():null;
}
...
function exportAttendance(){
  ...
  downloadCsv(`${currentClass()}_${range.label}_请假迟到统计.csv`,rows);toast(`已导出 ${affected.length} 名请假或迟到学生`);
}
...
async function syncDisplayAlerts(){
  ...
  try{
    ...
    if(callsChanged||announcementsChanged)render();
  }catch{}
}
