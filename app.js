...
const initial = {
  ...
  tasks: DEFAULT_TASKS, draft: {}, published: {}, announcements: [], assignments: [], homeworkStatus: {}, attendanceRecords: [], draftDirty: false, view: "login", picker: null, modal: null, popover: null,
};
let saved;
...
function announcementsPage() {
  ...
  return shell(`<div class="hero"><div><span class="eyebrow">班级信息</span><h1>班级公告</h1><p class="sub">仅支持文本公告，按照发布时间排列，并同步到教室展示模式。</p></div><button class="btn primary" data-action="add-announcement">＋ 发布公告</button></div><div class="notice-list">${items.length?items.map(item=>`<article class="card notice-item"><div><p>${esc(item.content)}</p><small>${esc(item.author)} · ${new Date(item.createdAt).toLocaleString('zh-CN')}</small></div><button class="btn small danger" data-delete-announcement="${item.id}">删除</button></article>`).join(''):`<div class="card empty">还没有班级公告。</div>`}</div>`,"announcements");
}
function homeworkPage() {
  const assignments=state.assignments||[];
  const current=assignments.find(a=>a.id===state.activeAssignmentId)||assignments[0];
  const counts={已交:0,未交:0,请假:0};
  if(current) state.students.forEach(s=>counts[state.homeworkStatus?.[`${current.id}:${s.id}`]||'未交']++);
  return shell(`<div class="hero"><div><span class="eyebrow">作业管理</span><h1>作业提交情况</h1><p class="sub">用于班主任所教学科，按学生记录已交、未交或请假。</p></div><div class="row"><button class="btn" data-action="export-homework" ${current?'':'disabled'}>导出 CSV</button><button class="btn primary" data-action="add-assignment">＋ 新建作业</button></div></div>${assignments.length?`<div class="card pad"><div class="toolbar"><label class="field-inline">当前作业 <select class="select" id="assignment-select">${assignments.map(a=>`<option value="${a.id}" ${a.id===current?.id?'selected':''}>${esc(a.subject)} · ${esc(a.title)}</option>`).join('')}</select></label><div class="toolbar-spacer"></div><span class="pill green">已交 ${counts.已交}</span><span class="pill orange">未交 ${counts.未交}</span><span class="pill">请假 ${counts.请假}</span></div><div class="table-wrap"><table><thead><tr><th>学生</th><th>学号</th><th>提交状态</th></tr></thead><tbody>${state.students.map(s=>{const status=state.homeworkStatus?.[`${current.id}:${s.id}`]||'未交';return `<tr><td><strong>${esc(s.name)}</strong></td><td>${esc(s.number||'—')}</td><td><div class="status-switch">${['已交','未交','请假'].map(x=>`<button class="btn small ${status===x?'primary':''}" data-homework-status="${current.id}|${s.id}|${x}">${x}</button>`).join('')}</div></td></tr>`}).join('')}</tbody></table></div></div>`:`<div class="card empty">还没有作业，点击“新建作业”开始登记。</div>`}`,"homework");
}
function attendancePage() {
  const records=[...(state.attendanceRecords||[])].sort((a,b)=>b.date.localeCompare(a.date));
  const leaveDays=records.filter(r=>r.kind!=='正常').reduce((n,r)=>n+Number(r.days||0),0);
  return shell(`<div class="hero"><div><span class="eyebrow">考勤管理</span><h1>学生考勤登记</h1><p class="sub">记录正常、事假、病假和离校；请假与值日排班联动。</p></div><div class="row"><button class="btn" data-action="export-attendance">导出 CSV</button><button class="btn primary" data-action="add-attendance-record">＋ 新增记录</button></div></div><div class="grid three"><div class="card stat"><div class="label">记录总数</div><div class="value">${records.length}</div></div><div class="card stat"><div class="label">请假人数</div><div class="value">${new Set(records.filter(r=>r.kind!=='正常').map(r=>r.studentId)).size}</div></div><div class="card stat"><div class="label">累计请假天数</div><div class="value">${leaveDays}</div></div></div><div class="card pad" style="margin-top:18px"><div class="table-wrap"><table><thead><tr><th>学生</th><th>状态</th><th>日期</th><th>时间段</th><th>天数</th><th>备注</th><th></th></tr></thead><tbody>${records.length?records.map(r=>`<tr><td><strong>${esc(state.students.find(s=>s.id===r.studentId)?.name||'已删除学生')}</strong></td><td><span class="pill ${r.kind==='病假'||r.kind==='事假'?'red':''}">${esc(r.kind)}</span></td><td>${esc(r.date)}</td><td>${esc(r.period||'全天')}</td><td>${esc(r.days||0)}</td><td>${esc(r.note||'—')}</td><td><button class="btn small danger" data-delete-attendance="${r.id}">删除</button></td></tr>`).join(''):`<tr><td colspan="7" class="empty">暂无考勤记录</td></tr>`}</tbody></table></div></div>`,"attendance");
}
...
function displayAnnouncementsPage(){const items=[...(state.announcements||[])].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));return `<div class="display-page"><div class="display-shell"><div class="display-head"><div class="display-title">${esc(currentClass())} · 班级公告</div><button class="btn" data-action="back-display">返回教室首页</button></div><div class="display-home"><div class="notice-list">${items.length?items.map(x=>`<article class="card notice-item"><div><p>${esc(x.content)}</p><small>${esc(x.author)} · ${new Date(x.createdAt).toLocaleString('zh-CN')}</small></div></article>`).join(''):`<div class="card empty">暂无公告</div>`}</div></div></div>${modal()}</div>`;}
function displayHomeworkPage(){const a=(state.assignments||[])[0];return `<div class="display-page"><div class="display-shell"><div class="display-head"><div class="display-title">${esc(currentClass())} · 作业提交</div><button class="btn" data-action="back-display">返回教室首页</button></div><div class="display-home">${a?`<div class="hero"><div><span class="eyebrow">${esc(a.subject)}</span><h1>${esc(a.title)}</h1><p class="sub">布置时间：${new Date(a.createdAt).toLocaleString('zh-CN')}</p></div></div><div class="card pad"><div class="table-wrap"><table><thead><tr><th>学生</th><th>状态</th></tr></thead><tbody>${state.students.map(s=>`<tr><td>${esc(s.name)}</td><td>${esc(state.homeworkStatus?.[`${a.id}:${s.id}`]||'未交')}</td></tr>`).join('')}</tbody></table></div></div>`:`<div class="card empty">暂无作业</div>`}</div></div>${modal()}</div>`;}
...
function modal() {
  ...
  if (state.modal.type==='announcement') return `<div class="modal-backdrop"><form class="modal" id="announcement-form"><div class="modal-head"><h2>发布班级公告</h2><button type="button" class="icon-btn" data-action="close-modal">×</button></div><div class="field"><label>公告内容 *</label><textarea class="input" name="content" rows="6" maxlength="500" placeholder="请输入公告内容" required></textarea></div><div class="form-actions"><button type="button" class="btn" data-action="close-modal">取消</button><button class="btn primary">发布公告</button></div></form></div>`;
  if (state.modal.type==='assignment') return `<div class="modal-backdrop"><form class="modal" id="assignment-form"><div class="modal-head"><h2>新建作业</h2><button type="button" class="icon-btn" data-action="close-modal">×</button></div><div class="field"><label>所教学科 *</label><input class="input" name="subject" placeholder="例如：语文" required /></div><div class="field"><label>作业内容 *</label><textarea class="input" name="title" rows="4" maxlength="300" required></textarea></div><div class="form-actions"><button type="button" class="btn" data-action="close-modal">取消</button><button class="btn primary">创建作业</button></div></form></div>`;
...
document.addEventListener("click", async e => {
  ...
  if(action==='display-homework'){state.view='display-homework';persist();render();}
  ...
  const delAnnouncement=e.target.closest('[data-delete-announcement]');if(delAnnouncement&&confirm('确定删除这条公告吗？')){state.announcements=(state.announcements||[]).filter(x=>x.id!==delAnnouncement.dataset.deleteAnnouncement);persist();render();return;}
...
document.addEventListener("change", e => {
  if(e.target.matches('#assignment-select')){state.activeAssignmentId=e.target.value;persist();render();return;}
...
document.addEventListener("submit", async e => {
  ...
  if(e.target.id==='announcement-form'){(state.announcements||=[]).push({id:crypto.randomUUID(),content:String(f.get('content')||'').trim(),author:teacherName(),createdAt:new Date().toISOString()});state.modal=null;persist();render();toast('公告已发布');return;}
  if(e.target.id==='assignment-form'){const item={id:crypto.randomUUID(),subject:String(f.get('subject')||'').trim(),title:String(f.get('title')||'').trim(),createdAt:new Date().toISOString()};(state.assignments||=[]).unshift(item);state.activeAssignmentId=item.id;state.modal=null;persist();render();toast('作业已创建');return;}
...
function exportAttendance(){downloadCsv(`${currentClass()}_考勤记录.csv`,[['学生','状态','日期','时间段','天数','备注'],...(state.attendanceRecords||[]).map(r=>[state.students.find(s=>s.id===r.studentId)?.name||'',r.kind,r.date,r.period,r.days,r.note])]);toast('考勤记录已导出');}
function exportHomework(){const a=(state.assignments||[]).find(x=>x.id===state.activeAssignmentId)||(state.assignments||[])[0];if(!a)return toast('暂无作业');downloadCsv(`${currentClass()}_${a.subject}_${a.title}.csv`,[['学生','学号','状态'],...state.students.map(s=>[s.name,s.number||'',state.homeworkStatus?.[`${a.id}:${s.id}`]||'未交'])]);toast('作业提交情况已导出');}
bootstrap();
