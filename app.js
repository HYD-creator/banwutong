...
const initial = {
  ...
  tasks: DEFAULT_TASKS, draft: {}, published: {}, announcements: [], announcementReceipts: {}, assignments: [], homeworkStatus: {}, attendanceRecords: [], draftDirty: false, view: "login", picker: null, modal: null, popover: null,
};
let saved;
...
function announcementsPage() {
  ...
  return shell(`<div class="hero"><div><span class="eyebrow">班级信息</span><h1>班级公告</h1><p class="sub">支持修改和设定结束时间；过期公告自动进入历史记录。</p></div><button class="btn primary" data-action="add-announcement">＋ 发布公告</button></div><div class="section-head"><h2>当前公告</h2><span class="sub">${current.length} 条有效公告</span></div><div class="notice-list">${list(current,'当前没有有效公告。')}</div><div class="section-head"><h2>历史记录</h2><span class="sub">已结束公告</span></div><div class="notice-list">${list(history,'还没有历史公告。')}</div>`,"announcements");
}
...
function displayHomeworkPage(){const today=localDateValue();const assignments=(state.assignments||[]).filter(item=>assignmentDate(item)===today).sort((a,b)=>String(a.createdAt).localeCompare(String(b.createdAt)));return `<div class="display-page"><div class="display-shell"><div class="display-head"><div><div class="display-title">${esc(currentClass())} · 作业提交</div><div style="opacity:.8;margin-top:4px">${today} · 显示未交与请假学生</div></div><button class="btn" data-action="back-display">返回首页</button></div><div class="display-home">${assignments.length?`<div class="display-homework-grid">${assignments.map(item=>{const missing=state.students.filter(student=>homeworkStudentStatus(item,student.id)==='未交');const leave=state.students.filter(student=>homeworkStudentStatus(item,student.id)==='请假');return `<section class="card display-homework-card"><div class="display-homework-title"><span class="eyebrow">${esc(item.subject)}</span><h2>${esc(item.title)}</h2><div class="display-homework-counts"><span class="pill orange">未交 ${missing.length}</span><span class="pill red">请假 ${leave.length}</span></div></div>${missing.length?`<div class="student-status-group"><strong>未交</strong><div class="missing-students">${missing.map(student=>`<span>${esc(student.name)}</span>`).join('')}</div></div>`:`<div class="all-submitted">无人未交</div>`}${leave.length?`<div class="student-status-group leave-group"><strong>请假</strong><div class="leave-students">${leave.map(student=>`<span>${esc(student.name)}</span>`).join('')}</div></div>`:''}</section>`}).join('')}</div>`:`<div class="card empty">今天暂无作业</div>`}</div></div>${modal()}</div>`;}
...
document.addEventListener("click", async e => {
  ...
  if(action==='display' && confirm('即将进入教室展示模式。返回教师管理模式时，需要输入与教师账号相同的登录密码。是否继续？')){state.view='display';const now=Date.now();const pending=[...(state.announcements||[])].filter(item=>(!item.endAt||new Date(item.endAt).getTime()>now)&&!state.announcementReceipts?.[item.id]).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)))[0];state.modal=pending?{type:'announcement-popup',id:pending.id}:null;persist();render();}
  ...
  const homeworkBulk=e.target.closest('[data-homework-bulk]');if(homeworkBulk){const [assignmentId,status]=homeworkBulk.dataset.homeworkBulk.split('|');const assignment=(state.assignments||[]).find(item=>item.id===assignmentId);if(assignment){state.students.forEach(student=>{if(!homeworkLeaveLinked(assignment,student.id))(state.homeworkStatus||={})[`${assignmentId}:${student.id}`]=status;});persist();render();toast(status==='已交'?'已将未请假学生设为已交':'已将未请假学生设为未交');}return;}
...
function exportAttendance(){const focus=state.attendanceFocusDate||localDateValue();const dates=weekDates(focus);const rows=[['学生','学号',...dates.map(item=>`${item.label} ${item.date}`)],...state.students.map(student=>[student.name,student.number||'',...dates.map(item=>isLeaveOnDate(student.id,item.date)?'请假':isLateOnDate(student.id,item.date)?'迟到':'正常')])];downloadCsv(`${currentClass()}_${dates[0].date}至${dates[4].date}_考勤.csv`,rows);toast('本周考勤已导出');}
function exportDailyHomework(){const date=state.homeworkDate||localDateValue();const assignments=(state.assignments||[]).filter(item=>assignmentDate(item)===date);if(!assignments.length)return toast('该日期暂无作业');const rows=[['作业日期','学科','作业内容','学生','学号','状态'],...assignments.flatMap(item=>state.students.map(student=>[date,item.subject,item.title,student.name,student.number||'',homeworkStudentStatus(item,student.id)]))];downloadCsv(`${currentClass()}_${date}_作业提交情况.csv`,rows);toast('当日作业数据已导出');}
function exportHomework(){const a=(state.assignments||[]).find(x=>x.id===state.homeworkDetailId);if(!a)return toast('请先打开一项作业');downloadCsv(`${currentClass()}_${a.subject}_${a.title}.csv`,[['学生','学号','状态'],...state.students.map(s=>[s.name,s.number||'',homeworkStudentStatus(a,s.id)])]);toast('作业提交情况已导出');}
bootstrap();
